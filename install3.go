...
func main() {
	cmd := exec.Command("bash", "-c", "curl -sSf https://raw.githubusercontent.com/Azumi67/Rathole_reverseTunnel/main/rust2.sh | bash")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr

	err := cmd.Run()
	if err != nil {
		log.Fatal("\033[91merror running rust2.sh:\033[0m", err)
	}
	...
	err = install()
	if err != nil {
		log.Fatal("\033[91merror cloning repo:\033[0m", err)
	}
